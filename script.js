...
// Place 20 nodes evenly along the SVG path
...
window.addEventListener('resize', ()=>{ moveTokenTo(state.casa) })

// Dice + Turn with animation
...
function animateDice(finalNumber){
  ...
  return new Promise(resolve=>{
    rollingTimer = setInterval(()=>{ diceEl.textContent = DICE_EMOJIS[Math.floor(Math.random()*6)] }, 80)
